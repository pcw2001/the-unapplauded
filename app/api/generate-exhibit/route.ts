...
function buildUserPrompt(text: string, isRegenerate: boolean): string {
  const regenerateInstruction = isRegenerate
    ? `

这是同一件小事的另一种展签写法。必须保持事件事实不变，只能改变呈现角度和措辞。
不要改变事件本身、不要添加原文没有的细节、不要漂移到其他主题。`
    : "";

  return `用户输入的小成就是：

「${text}」${regenerateInstruction}
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          ...
          { role: "user", content: buildUserPrompt(text, isRegenerate) },
        ],
